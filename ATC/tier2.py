    ...
    for i in range(len(results)):
        ...
        for flight_id in flightlist:
            ...
            if flight_id != flight_num:
                if ((next_wp.lat - flight_wp.lat)**2 + (next_wp.lon - flight_wp.lon)**2)**0.5 < 0.03:
                    if abs(new_parameters['next_altitude'] - flights_to_altitudes[flight_id]) < 300:
        ...
        if max_alt > 0:
            new_parameters["next_altitude"] = max_alt + 350
        else:
            a = 1
