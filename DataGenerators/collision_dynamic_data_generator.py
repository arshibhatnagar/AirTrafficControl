...
if __name__ == "__main__":
    num_points = 2
    file = open("Data/reverse.txt")
    route_list = [json.loads(line) for line in file]
    indices = [0,1]
    flight_number = [1902, 648]
    dynamic_data = []
    for i in indices:
        flight_num = "AI" + str(flight_number[i])
        routes = route_list[i]
        ...
        for i in range(len(waypoints)-1):
            ...
            for i in range(num_iter-1):
                ...
                altitude = 20000 + np.random.randint(-100,140)
    ...
    with open("Data/collision_dynamic_data.json", 'w') as file:
